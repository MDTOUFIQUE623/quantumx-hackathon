/**
 * @fileoverview eFrodoKEM-1344-SHAKE KEM algorithm implementation
 * @module algorithms/kem/frodokem/efrodokem-1344-shake
 * @description
 * eFrodoKEM-1344-SHAKE is a lattice-based key encapsulation mechanism providing NIST security level 5.
...
/**
 * EFRODOKEM-1344-SHAKE-INFO algorithm constants and metadata
 * @type {{readonly name: 'eFrodoKEM-1344-SHAKE', readonly identifier: 'eFrodoKEM-1344-SHAKE', readonly type: 'kem', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 21520, readonly secretKey: 43088, readonly ciphertext: 21632, readonly sharedSecret: 32}}}
 */
export const EFRODOKEM_1344_SHAKE_INFO = {
  name: 'eFrodoKEM-1344-SHAKE',
  identifier: 'eFrodoKEM-1344-SHAKE',
  type: 'kem',
  securityLevel: 5,
  standardized: false,
  description: 'eFrodoKEM-1344-SHAKE lattice-based KEM (NIST Level 5, 256-bit quantum security, SHAKE)',
  keySize: {
    publicKey: 21520,
    secretKey: 43088,
    ciphertext: 21632,
...
/**
 * Factory function to create a eFrodoKEM-1344-SHAKE KEM instance
 *
 * @async
 * @function createEFrodoKEM1344SHAKE
 * @returns {Promise<EFrodoKEM1344SHAKE>} Initialized eFrodoKEM-1344-SHAKE instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createEFrodoKEM1344SHAKE } from '@oqs/liboqs-js';
 *
 * const kem = await createEFrodoKEM1344SHAKE();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createEFrodoKEM1344SHAKE() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = EFRODOKEM_1344_SHAKE_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('eFrodoKEM-1344-SHAKE', 'Failed to create KEM instance');
  }

  return new EFrodoKEM1344SHAKE(wasmModule, kemPtr);
}

/**
 * eFrodoKEM-1344-SHAKE key encapsulation mechanism wrapper class
 *
 * @class EFrodoKEM1344SHAKE
 * @description
 * High-level wrapper for eFrodoKEM-1344-SHAKE KEM operations. Provides secure key generation,
 ...
 * kem.destroy();
 */
export class EFrodoKEM1344SHAKE {
  ...
  /**
   * Generate a new eFrodoKEM-1344-SHAKE keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(EFRODOKEM_1344_SHAKE_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(EFRODOKEM_1344_SHAKE_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'eFrodoKEM-1344-SHAKE', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(EFRODOKEM_1344_SHAKE_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(EFRODOKEM_1344_SHAKE_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'eFrodoKEM-1344-SHAKE', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'eFrodoKEM-1344-SHAKE', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof EFRODOKEM_1344_SHAKE_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'eFrodoKEM-1344-SHAKE'
   ...
   */
  get info() {
    return EFRODOKEM_1344_SHAKE_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'eFrodoKEM-1344-SHAKE');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== EFRODOKEM_1344_SHAKE_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${EFRODOKEM_1344_SHAKE_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== EFRODOKEM_1344_SHAKE_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${EFRODOKEM_1344_SHAKE_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'eFrodoKEM-1344-SHAKE'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== EFRODOKEM_1344_SHAKE_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${EFRODOKEM_1344_SHAKE_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'eFrodoKEM-1344-SHAKE'
      );
    }
  }
