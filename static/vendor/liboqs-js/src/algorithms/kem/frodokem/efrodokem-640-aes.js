/**
 * @fileoverview eFrodoKEM-640-AES KEM algorithm implementation
 * @module algorithms/kem/frodokem/efrodokem-640-aes
 * @description
 * eFrodoKEM-640-AES is a lattice-based key encapsulation mechanism providing NIST security level 1.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/frodokem-640-aes.deno.js`
    : `../../../../dist/frodokem-640-aes.min.js`;
...
/**
 * EFRODOKEM-640-AES-INFO algorithm constants and metadata
 * @type {{readonly name: 'eFrodoKEM-640-AES', readonly identifier: 'eFrodoKEM-640-AES', readonly type: 'kem', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 9616, readonly secretKey: 19888, readonly ciphertext: 9720, readonly sharedSecret: 16}}}
 */
export const EFRODOKEM_640_AES_INFO = {
  name: 'eFrodoKEM-640-AES',
  identifier: 'eFrodoKEM-640-AES',
  type: 'kem',
  securityLevel: 1,
  standardized: false,
  description: 'eFrodoKEM-640-AES lattice-based KEM (NIST Level 1, 128-bit quantum security, AES)',
  keySize: {
    publicKey: 9616,
    secretKey: 19888,
    ciphertext: 9720,
...
/**
 * Factory function to create a eFrodoKEM-640-AES KEM instance
 *
 * @async
 * @function createEFrodoKEM640AES
 * @returns {Promise<EFrodoKEM640AES>} Initialized eFrodoKEM-640-AES instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createEFrodoKEM640AES } from '@oqs/liboqs-js';
 *
 * const kem = await createEFrodoKEM640AES();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createEFrodoKEM640AES() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = EFRODOKEM_640_AES_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('eFrodoKEM-640-AES', 'Failed to create KEM instance');
  }

  return new EFrodoKEM640AES(wasmModule, kemPtr);
}

/**
 * eFrodoKEM-640-AES key encapsulation mechanism wrapper class
 *
 * @class EFrodoKEM640AES
 * @description
 * High-level wrapper for eFrodoKEM-640-AES KEM operations. Provides secure key generation,
...
export class EFrodoKEM640AES {
  ...
  /**
   * Generate a new eFrodoKEM-640-AES keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(EFRODOKEM_640_AES_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(EFRODOKEM_640_AES_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'eFrodoKEM-640-AES', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(EFRODOKEM_640_AES_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(EFRODOKEM_640_AES_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'eFrodoKEM-640-AES', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'eFrodoKEM-640-AES', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof EFRODOKEM_640_AES_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'eFrodoKEM-640-AES'
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'eFrodoKEM-640-AES');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== EFRODOKEM_640_AES_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${EFRODOKEM_640_AES_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'eFrodoKEM-640-AES'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== EFRODOKEM_640_AES_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${EFRODOKEM_640_AES_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'eFrodoKEM-640-AES'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== EFRODOKEM_640_AES_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${EFRODOKEM_640_AES_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'eFrodoKEM-640-AES'
      );
    }
  }
