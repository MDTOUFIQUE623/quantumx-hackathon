/**
 * @fileoverview eFrodoKEM-640-SHAKE KEM algorithm implementation
 * @module algorithms/kem/frodokem/efrodokem-640-shake
 * @description
 * eFrodoKEM-640-SHAKE is a lattice-based key encapsulation mechanism providing NIST security level 1.
 * It uses learning with errors (LWE) and SHAKE for pseudorandom generation.
 *
 * Key features:
 * - Lattice-based cryptography (LWE problem)
 * - Security Level 1 (128-bit classical, quantum-resistant)
 * - Conservative security margin
 * - SHAKE-based pseudorandom generation
 *
...
/**
 * EFRODOKEM-640-SHAKE-INFO algorithm constants and metadata
 * @type {{readonly name: 'eFrodoKEM-640-SHAKE', readonly identifier: 'eFrodoKEM-640-SHAKE', readonly type: 'kem', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 9616, readonly secretKey: 19888, readonly ciphertext: 9720, readonly sharedSecret: 16}}}
 */
export const EFRODOKEM_640_SHAKE_INFO = {
  name: 'eFrodoKEM-640-SHAKE',
  identifier: 'eFrodoKEM-640-SHAKE',
  type: 'kem',
  securityLevel: 1,
  standardized: false,
  description: 'eFrodoKEM-640-SHAKE lattice-based KEM (NIST Level 1, 128-bit quantum security, SHAKE)',
  keySize: {
    publicKey: 9616,
    secretKey: 19888,
    ciphertext: 9720,
...
/**
 * Factory function to create a eFrodoKEM-640-SHAKE KEM instance
 *
 * @async
 * @function createEFrodoKEM640SHAKE
 * @returns {Promise<EFrodoKEM640SHAKE>} Initialized eFrodoKEM-640-SHAKE instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createEFrodoKEM640SHAKE } from '@oqs/liboqs-js';
 *
 * const kem = await createEFrodoKEM640SHAKE();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createEFrodoKEM640SHAKE() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = EFRODOKEM_640_SHAKE_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('eFrodoKEM-640-SHAKE', 'Failed to create KEM instance');
  }

  return new EFrodoKEM640SHAKE(wasmModule, kemPtr);
}

/**
 * eFrodoKEM-640-SHAKE key encapsulation mechanism wrapper class
 *
 * @class EFrodoKEM640SHAKE
 * @description
 * High-level wrapper for eFrodoKEM-640-SHAKE KEM operations. Provides secure key generation,
 ...
 * kem.destroy();
 */
export class EFrodoKEM640SHAKE {
  ...
  /**
   * Generate a new eFrodoKEM-640-SHAKE keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(EFRODOKEM_640_SHAKE_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(EFRODOKEM_640_SHAKE_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'eFrodoKEM-640-SHAKE', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(EFRODOKEM_640_SHAKE_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(EFRODOKEM_640_SHAKE_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'eFrodoKEM-640-SHAKE', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'eFrodoKEM-640-SHAKE', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof EFRODOKEM_640_SHAKE_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'eFrodoKEM-640-SHAKE'
   * console.log(kem.info.securityLevel);  // 1
   * console.log(kem.info.keySize);        // { publicKey: 9616, secretKey: 19888, ciphertext: 9720, sharedSecret: 16 }
   */
  get info() {
    return EFRODOKEM_640_SHAKE_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'eFrodoKEM-640-SHAKE');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== EFRODOKEM_640_SHAKE_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${EFRODOKEM_640_SHAKE_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'eFrodoKEM-640-SHAKE'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== EFRODOKEM_640_SHAKE_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${EFRODOKEM_640_SHAKE_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'eFrodoKEM-640-SHAKE'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== EFRODOKEM_640_SHAKE_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${EFRODOKEM_640_SHAKE_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'eFrodoKEM-640-SHAKE'
      );
    }
  }
