/**
 * @fileoverview eFrodoKEM-976-AES KEM algorithm implementation
 * @module algorithms/kem/frodokem/efrodokem-976-aes
 * @description
 * eFrodoKEM-976-AES is a lattice-based key encapsulation mechanism providing NIST security level 3.
...
/**
 * EFRODOKEM-976-AES-INFO algorithm constants and metadata
 * @type {{readonly name: 'eFrodoKEM-976-AES', readonly identifier: 'eFrodoKEM-976-AES', readonly type: 'kem', readonly securityLevel: 3, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 15632, readonly secretKey: 31296, readonly ciphertext: 15744, readonly sharedSecret: 24}}}
 */
export const EFRODOKEM_976_AES_INFO = {
  name: 'eFrodoKEM-976-AES',
  identifier: 'eFrodoKEM-976-AES',
  type: 'kem',
  securityLevel: 3,
  standardized: false,
  description: 'eFrodoKEM-976-AES lattice-based KEM (NIST Level 3, 192-bit quantum security, AES)',
  keySize: {
    publicKey: 15632,
    secretKey: 31296,
    ciphertext: 15744,
...
/**
 * Factory function to create a eFrodoKEM-976-AES KEM instance
 *
 * @async
 * @function createEFrodoKEM976AES
 * @returns {Promise<EFrodoKEM976AES>} Initialized eFrodoKEM-976-AES instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createEFrodoKEM976AES } from '@oqs/liboqs-js';
 *
 * const kem = await createEFrodoKEM976AES();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createEFrodoKEM976AES() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = EFRODOKEM_976_AES_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('eFrodoKEM-976-AES', 'Failed to create KEM instance');
  }

  return new EFrodoKEM976AES(wasmModule, kemPtr);
}

/**
 * eFrodoKEM-976-AES key encapsulation mechanism wrapper class
 *
 * @class EFrodoKEM976AES
 * @description
 * High-level wrapper for eFrodoKEM-976-AES KEM operations. Provides secure key generation,
...
export class EFrodoKEM976AES {
  ...
  /**
   * Generate a new eFrodoKEM-976-AES keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(EFRODOKEM_976_AES_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(EFRODOKEM_976_AES_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'eFrodoKEM-976-AES', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(EFRODOKEM_976_AES_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(EFRODOKEM_976_AES_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'eFrodoKEM-976-AES', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'eFrodoKEM-976-AES', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof EFRODOKEM_976_AES_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'eFrodoKEM-976-AES'
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'eFrodoKEM-976-AES');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== EFRODOKEM_976_AES_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${EFRODOKEM_976_AES_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'eFrodoKEM-976-AES'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== EFRODOKEM_976_AES_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${EFRODOKEM_976_AES_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'eFrodoKEM-976-AES'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== EFRODOKEM_976_AES_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${EFRODOKEM_976_AES_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'eFrodoKEM-976-AES'
      );
    }
  }
