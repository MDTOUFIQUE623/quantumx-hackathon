/**
 * @fileoverview eFrodoKEM-976-SHAKE KEM algorithm implementation
 * @module algorithms/kem/frodokem/efrodokem-976-shake
 * @description
 * eFrodoKEM-976-SHAKE is a lattice-based key encapsulation mechanism providing NIST security level 3.
...
/**
 * EFRODOKEM-976-SHAKE-INFO algorithm constants and metadata
 * @type {{readonly name: 'eFrodoKEM-976-SHAKE', readonly identifier: 'eFrodoKEM-976-SHAKE', readonly type: 'kem', readonly securityLevel: 3, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 15632, readonly secretKey: 31296, readonly ciphertext: 15744, readonly sharedSecret: 24}}}
 */
export const EFRODOKEM_976_SHAKE_INFO = {
  name: 'eFrodoKEM-976-SHAKE',
  identifier: 'eFrodoKEM-976-SHAKE',
  type: 'kem',
  securityLevel: 3,
  standardized: false,
  description: 'eFrodoKEM-976-SHAKE lattice-based KEM (NIST Level 3, 192-bit quantum security, SHAKE)',
  keySize: {
    publicKey: 15632,
    secretKey: 31296,
    ciphertext: 15744,
...
/**
 * Factory function to create a eFrodoKEM-976-SHAKE KEM instance
 *
 * @async
 * @function createEFrodoKEM976SHAKE
 * @returns {Promise<EFrodoKEM976SHAKE>} Initialized eFrodoKEM-976-SHAKE instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createEFrodoKEM976SHAKE } from '@oqs/liboqs-js';
 *
 * const kem = await createEFrodoKEM976SHAKE();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createEFrodoKEM976SHAKE() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = EFRODOKEM_976_SHAKE_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('eFrodoKEM-976-SHAKE', 'Failed to create KEM instance');
  }

  return new EFrodoKEM976SHAKE(wasmModule, kemPtr);
}

/**
 * eFrodoKEM-976-SHAKE key encapsulation mechanism wrapper class
 *
 * @class EFrodoKEM976SHAKE
 * @description
 * High-level wrapper for eFrodoKEM-976-SHAKE KEM operations. Provides secure key generation,
 ...
 * kem.destroy();
 */
export class EFrodoKEM976SHAKE {
  ...
  /**
   * Generate a new eFrodoKEM-976-SHAKE keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(EFRODOKEM_976_SHAKE_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(EFRODOKEM_976_SHAKE_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'eFrodoKEM-976-SHAKE', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(EFRODOKEM_976_SHAKE_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(EFRODOKEM_976_SHAKE_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'eFrodoKEM-976-SHAKE', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'eFrodoKEM-976-SHAKE', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof EFRODOKEM_976_SHAKE_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'eFrodoKEM-976-SHAKE'
   ...
   */
  get info() {
    return EFRODOKEM_976_SHAKE_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'eFrodoKEM-976-SHAKE');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== EFRODOKEM_976_SHAKE_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${EFRODOKEM_976_SHAKE_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'eFrodoKEM-976-SHAKE'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== EFRODOKEM_976_SHAKE_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${EFRODOKEM_976_SHAKE_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'eFrodoKEM-976-SHAKE'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== EFRODOKEM_976_SHAKE_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${EFRODOKEM_976_SHAKE_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'eFrodoKEM-976-SHAKE'
      );
    }
  }
