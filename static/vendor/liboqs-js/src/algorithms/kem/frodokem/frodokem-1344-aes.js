/**
 * @fileoverview FrodoKEM-1344-AES KEM algorithm implementation
 * @module algorithms/kem/frodokem/frodokem-1344-aes
 * @description
 * FrodoKEM-1344-AES is a lattice-based key encapsulation mechanism providing NIST security level 5.
...
/**
 * FRODOKEM-1344-AES-INFO algorithm constants and metadata
 * @type {{readonly name: 'FrodoKEM-1344-AES', readonly identifier: 'FrodoKEM-1344-AES', readonly type: 'kem', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 21520, readonly secretKey: 43088, readonly ciphertext: 21696, readonly sharedSecret: 32}}}
 */
export const FRODOKEM_1344_AES_INFO = {
  name: 'FrodoKEM-1344-AES',
  identifier: 'FrodoKEM-1344-AES',
  type: 'kem',
  securityLevel: 5,
  standardized: false,
  description: 'FrodoKEM-1344-AES lattice-based KEM (NIST Level 5, 256-bit quantum security, AES)',
  keySize: {
    publicKey: 21520,
    secretKey: 43088,
    ciphertext: 21696,
...
/**
 * Factory function to create a FrodoKEM-1344-AES KEM instance
 *
 * @async
 * @function createFrodoKEM1344AES
 * @returns {Promise<FrodoKEM1344AES>} Initialized FrodoKEM-1344-AES instance
 ...
 * kem.destroy();
 */
export async function createFrodoKEM1344AES() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = FRODOKEM_1344_AES_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('FrodoKEM-1344-AES', 'Failed to create KEM instance');
  }

  return new FrodoKEM1344AES(wasmModule, kemPtr);
}
 ...
 * @description
 * High-level wrapper for FrodoKEM-1344-AES KEM operations. Provides secure key generation,
...
export class FrodoKEM1344AES {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(FRODOKEM_1344_AES_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(FRODOKEM_1344_AES_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'FrodoKEM-1344-AES', 'Key generation failed');
      }
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(FRODOKEM_1344_AES_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(FRODOKEM_1344_AES_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'FrodoKEM-1344-AES', 'Encapsulation failed');
      }
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'FrodoKEM-1344-AES', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof FRODOKEM_1344_AES_INFO} Algorithm metadata
   *
   * @example
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'FrodoKEM-1344-AES');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== FRODOKEM_1344_AES_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${FRODOKEM_1344_AES_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'FrodoKEM-1344-AES'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== FRODOKEM_1344_AES_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${FRODOKEM_1344_AES_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'FrodoKEM-1344-AES'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== FRODOKEM_1344_AES_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${FRODOKEM_1344_AES_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'FrodoKEM-1344-AES'
      );
    }
  }
