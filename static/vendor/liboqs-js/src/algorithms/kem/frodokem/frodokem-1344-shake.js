/**
 * @fileoverview FrodoKEM-1344-SHAKE KEM algorithm implementation
 * @module algorithms/kem/frodokem/frodokem-1344-shake
 * @description
 * FrodoKEM-1344-SHAKE is a lattice-based key encapsulation mechanism providing NIST security level 5.
 ...
 * @type {{readonly name: 'FrodoKEM-1344-SHAKE', readonly identifier: 'FrodoKEM-1344-SHAKE', readonly type: 'kem', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 21520, readonly secretKey: 43088, readonly ciphertext: 21696, readonly sharedSecret: 32}}}
 */
export const FRODOKEM_1344_SHAKE_INFO = {
  name: 'FrodoKEM-1344-SHAKE',
  identifier: 'FrodoKEM-1344-SHAKE',
  type: 'kem',
  securityLevel: 5,
  standardized: false,
  description: 'FrodoKEM-1344-SHAKE lattice-based KEM (NIST Level 5, 256-bit quantum security, SHAKE)',
  keySize: {
    publicKey: 21520,
    secretKey: 43088,
    ciphertext: 21696,
...
/**
 * Factory function to create a FrodoKEM-1344-SHAKE KEM instance
 *
 * @async
 * @function createFrodoKEM1344SHAKE
 * @returns {Promise<FrodoKEM1344SHAKE>} Initialized FrodoKEM-1344-SHAKE instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createFrodoKEM1344SHAKE } from '@oqs/liboqs-js';
 *
 * const kem = await createFrodoKEM1344SHAKE();
 * const { publicKey, secretKey } = kem.generateKeyPair();
 * kem.destroy();
 */
export async function createFrodoKEM1344SHAKE() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = FRODOKEM_1344_SHAKE_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('FrodoKEM-1344-SHAKE', 'Failed to create KEM instance');
  }

  return new FrodoKEM1344SHAKE(wasmModule, kemPtr);
}

/**
 * FrodoKEM-1344-SHAKE key encapsulation mechanism wrapper class
 *
 * @class FrodoKEM1344SHAKE
 * @description
 * High-level wrapper for FrodoKEM-1344-SHAKE KEM operations. Provides secure key generation,
 ...
 * kem.destroy();
 */
export class FrodoKEM1344SHAKE {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(FRODOKEM_1344_SHAKE_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(FRODOKEM_1344_SHAKE_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'FrodoKEM-1344-SHAKE', 'Key generation failed');
      }
   ...
   * @example
   * const { ciphertext, sharedSecret } = kem.encapsulate(publicKey);
   * console.log('Ciphertext:', ciphertext.length);      // 21696 bytes
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(FRODOKEM_1344_SHAKE_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(FRODOKEM_1344_SHAKE_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'FrodoKEM-1344-SHAKE', 'Encapsulation failed');
      }
   ...
   *
   * @async
   * @param {Uint8Array} ciphertext - Ciphertext (21696 bytes)
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'FrodoKEM-1344-SHAKE', 'Decapsulation failed');
      }
   ...
   * console.log(kem.info.keySize);        // { publicKey: 21520, secretKey: 43088, ciphertext: 21696, sharedSecret: 32 }
   */
  get info() {
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'FrodoKEM-1344-SHAKE');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== FRODOKEM_1344_SHAKE_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${FRODOKEM_1344_SHAKE_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'FrodoKEM-1344-SHAKE'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== FRODOKEM_1344_SHAKE_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${FRODOKEM_1344_SHAKE_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'FrodoKEM-1344-SHAKE'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== FRODOKEM_1344_SHAKE_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${FRODOKEM_1344_SHAKE_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'FrodoKEM-1344-SHAKE'
      );
    }
  }
