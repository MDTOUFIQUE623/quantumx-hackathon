/**
 * @fileoverview NTRU-HRSS-1373 KEM algorithm implementation
 * @module algorithms/kem/ntru/ntru-hrss-1373
 * @description
 * NTRU-HRSS-1373 is a lattice-based key encapsulation mechanism from the NTRU-HRSS (Highest Robustness Secure) family.
...
/**
 * NTRU-HRSS-1373-INFO algorithm constants and metadata
 * @type {{readonly name: 'NTRU-HRSS-1373', readonly identifier: 'NTRU-HRSS-1373', readonly type: 'kem', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 2401, readonly secretKey: 2983, readonly ciphertext: 2401, readonly sharedSecret: 32}}}
 */
export const NTRU_HRSS_1373_INFO = {
  name: 'NTRU-HRSS-1373',
  identifier: 'NTRU-HRSS-1373',
  type: 'kem',
  securityLevel: 5,
  standardized: false,
  description: 'NTRU-HRSS-1373 NTRU-HRSS (Highest Robustness Secure) (NIST Level 5, 256-bit quantum security)',
  keySize: {
    publicKey: 2401,
    secretKey: 2983,
    ciphertext: 2401,
...
/**
 * Factory function to create a NTRU-HRSS-1373 KEM instance
 *
 * @async
 * @function createNTRUHrss1373
 * @returns {Promise<NTRUHrss1373>} Initialized NTRU-HRSS-1373 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createNTRUHrss1373 } from '@oqs/liboqs-js';
 *
...
export async function createNTRUHrss1373() {
  ...
  const algoName = NTRU_HRSS_1373_INFO.identifier;
  ...
  if (!kemPtr) {
    throw new LibOQSInitError('NTRU-HRSS-1373', 'Failed to create KEM instance');
  }

  return new NTRUHrss1373(wasmModule, kemPtr);
}
 ...
 * @class NTRUHrss1373
 * @description
 * High-level wrapper for NTRU-HRSS-1373 KEM operations. Provides secure key generation,
...
export class NTRUHrss1373 {
   ...
   * @example
   * const { publicKey, secretKey } = kem.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 2401 bytes
   * console.log('Secret key:', secretKey.length);  // 2983 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(NTRU_HRSS_1373_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(NTRU_HRSS_1373_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'NTRU-HRSS-1373', 'Key generation failed');
      }
   ...
   *
   * @async
   * @param {Uint8Array} publicKey - Public key for encapsulation (2401 bytes)
   ...
   * console.log('Ciphertext:', ciphertext.length);     // 2401 bytes
  ...
  encapsulate(publicKey) {
    ...
    const ciphertext = new Uint8Array(NTRU_HRSS_1373_INFO.keySize.ciphertext);
    const sharedSecret = new Uint8Array(NTRU_HRSS_1373_INFO.keySize.sharedSecret);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('encapsulate', 'NTRU-HRSS-1373', 'Encapsulation failed');
      }
   ...
   *
   * @async
   * @param {Uint8Array} ciphertext - Ciphertext to decapsulate (2401 bytes)
   * @param {Uint8Array} secretKey - Secret key for decapsulation (2983 bytes)
  ...
  decapsulate(ciphertext, secretKey) {
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('decapsulate', 'NTRU-HRSS-1373', 'Decapsulation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof NTRU_HRSS_1373_INFO} Algorithm metadata
   *
   * @example
   * console.log(kem.info.name);           // 'NTRU-HRSS-1373'
   * console.log(kem.info.securityLevel);  // 5
   * console.log(kem.info.keySize);        // { publicKey: 2401, secretKey: 2983, ciphertext: 2401, sharedSecret: 32 }
   */
  get info() {
    return NTRU_HRSS_1373_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'NTRU-HRSS-1373');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== NTRU_HRSS_1373_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${NTRU_HRSS_1373_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'NTRU-HRSS-1373'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== NTRU_HRSS_1373_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${NTRU_HRSS_1373_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'NTRU-HRSS-1373'
      );
    }
  }
  ...
  #validateCiphertext(ciphertext) {
    if (!isUint8Array(ciphertext) || ciphertext.length !== NTRU_HRSS_1373_INFO.keySize.ciphertext) {
      throw new LibOQSValidationError(
        `Invalid ciphertext: expected ${NTRU_HRSS_1373_INFO.keySize.ciphertext} bytes, got ${ciphertext?.length ?? 'null'}`,
        'NTRU-HRSS-1373'
      );
    }
  }
