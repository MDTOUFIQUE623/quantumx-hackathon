/**
 * @fileoverview CROSS-rsdp-128-balanced signature algorithm implementation
 * @module algorithms/sig/cross/cross-rsdp-128-balanced
 * @description
 * CROSS-rsdp-128-balanced is a code-based signature scheme providing NIST security level 1.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/cross-rsdp-128-balanced.deno.js`
    : `../../../../dist/cross-rsdp-128-balanced.min.js`;
...
/**
 * CROSS-RSDP-128-BALANCED-INFO algorithm constants and metadata
 * @type {{readonly name: 'CROSS-rsdp-128-balanced', readonly identifier: 'CROSS-rsdp-128-balanced', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 77, readonly secretKey: 32, readonly signature: 13152}}}
 */
export const CROSS_RSDP_128_BALANCED_INFO = {
  name: 'CROSS-rsdp-128-balanced',
  identifier: 'CROSS-rsdp-128-balanced',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'CROSS-rsdp-128-balanced code-based signature (NIST Level 1, 128-bit quantum security, balanced)',
  keySize: {
    publicKey: 77,
    secretKey: 32,
    signature: 13152
...
/**
 * Factory function to create a CROSS-rsdp-128-balanced signature instance
 *
 * @async
 * @function createCrossRsdp128Balanced
 * @returns {Promise<CrossRsdp128Balanced>} Initialized CROSS-rsdp-128-balanced instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createCrossRsdp128Balanced } from '@oqs/liboqs-js';
 *
 * const sig = await createCrossRsdp128Balanced();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createCrossRsdp128Balanced() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = CROSS_RSDP_128_BALANCED_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('CROSS-rsdp-128-balanced', 'Failed to create SIG instance');
  }

  return new CrossRsdp128Balanced(wasmModule, sigPtr);
}

/**
 * CROSS-rsdp-128-balanced signature algorithm wrapper class
 *
 * @class CrossRsdp128Balanced
 * @description
 * High-level wrapper for CROSS-rsdp-128-balanced signature operations. Provides secure key generation,
 ...
 *
 * @example
 * const sig = await createCrossRsdp128Balanced(moduleFactory);
 *
 * // Generate keypair
 * const { publicKey, secretKey } = sig.generateKeyPair();
 *
 ...
 * sig.destroy();
 */
export class CrossRsdp128Balanced {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(CROSS_RSDP_128_BALANCED_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(CROSS_RSDP_128_BALANCED_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'CROSS-rsdp-128-balanced', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Signature (variable length, max 13152 bytes)
   * @throws {LibOQSError} If instance is destroyed
  ...
  sign(message, secretKey) {
    ...
    const secretKeyPtr = this.#wasmModule._malloc(secretKey.length);
    const signaturePtr = this.#wasmModule._malloc(CROSS_RSDP_128_BALANCED_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'CROSS-rsdp-128-balanced', 'Signature generation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof CROSS_RSDP_128_BALANCED_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'CROSS-rsdp-128-balanced'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 77, secretKey: 32, signature: 13152 }
   */
  get info() {
    return CROSS_RSDP_128_BALANCED_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'CROSS-rsdp-128-balanced');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== CROSS_RSDP_128_BALANCED_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${CROSS_RSDP_128_BALANCED_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'CROSS-rsdp-128-balanced'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== CROSS_RSDP_128_BALANCED_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${CROSS_RSDP_128_BALANCED_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'CROSS-rsdp-128-balanced'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > CROSS_RSDP_128_BALANCED_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${CROSS_RSDP_128_BALANCED_INFO.keySize.signature} bytes, got ${signature.length}`,
        'CROSS-rsdp-128-balanced'
      );
    }
  }
