/**
 * @fileoverview CROSS-rsdp-128-small signature algorithm implementation
 * @module algorithms/sig/cross/cross-rsdp-128-small
 * @description
 * CROSS-rsdp-128-small is a code-based signature scheme providing NIST security level 1.
...
/**
 * CROSS-RSDP-128-SMALL-INFO algorithm constants and metadata
 * @type {{readonly name: 'CROSS-rsdp-128-small', readonly identifier: 'CROSS-rsdp-128-small', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 77, readonly secretKey: 32, readonly signature: 12432}}}
 */
export const CROSS_RSDP_128_SMALL_INFO = {
  name: 'CROSS-rsdp-128-small',
  identifier: 'CROSS-rsdp-128-small',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'CROSS-rsdp-128-small code-based signature (NIST Level 1, 128-bit quantum security, small)',
  keySize: {
    publicKey: 77,
    secretKey: 32,
    signature: 12432
...
/**
 * Factory function to create a CROSS-rsdp-128-small signature instance
 *
 * @async
 * @function createCrossRsdp128Small
 * @returns {Promise<CrossRsdp128Small>} Initialized CROSS-rsdp-128-small instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createCrossRsdp128Small } from '@oqs/liboqs-js';
 *
 * const sig = await createCrossRsdp128Small();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createCrossRsdp128Small() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = CROSS_RSDP_128_SMALL_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('CROSS-rsdp-128-small', 'Failed to create SIG instance');
  }

  return new CrossRsdp128Small(wasmModule, sigPtr);
}

/**
 * CROSS-rsdp-128-small signature algorithm wrapper class
 *
 * @class CrossRsdp128Small
 * @description
 * High-level wrapper for CROSS-rsdp-128-small signature operations. Provides secure key generation,
 ...
 *
 * @example
 * const sig = await createCrossRsdp128Small(moduleFactory);
 *
 * // Generate keypair
 * const { publicKey, secretKey } = sig.generateKeyPair();
 *
 ...
 * sig.destroy();
 */
export class CrossRsdp128Small {
  ...
  /**
   * Generate a new CROSS-rsdp-128-small keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(CROSS_RSDP_128_SMALL_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(CROSS_RSDP_128_SMALL_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'CROSS-rsdp-128-small', 'Key generation failed');
      }
  ...
  /**
   * Sign a message with CROSS-rsdp-128-small
   *
   * @async
   * @param {Uint8Array} message - Message to sign (any length)
   * @param {Uint8Array} secretKey - Secret key (32 bytes)
   * @returns {Uint8Array} Signature (variable length, max 12432 bytes)
   * @throws {LibOQSError} If instance is destroyed
  ...
  sign(message, secretKey) {
    ...
    const secretKeyPtr = this.#wasmModule._malloc(secretKey.length);
    const signaturePtr = this.#wasmModule._malloc(CROSS_RSDP_128_SMALL_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'CROSS-rsdp-128-small', 'Signature generation failed');
      }
  ...
  /**
   * Verify a CROSS-rsdp-128-small signature
   *
   * @async
   ...
   *
   * @readonly
   * @returns {typeof CROSS_RSDP_128_SMALL_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'CROSS-rsdp-128-small'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 77, secretKey: 32, signature: 12432 }
   */
  get info() {
    return CROSS_RSDP_128_SMALL_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'CROSS-rsdp-128-small');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== CROSS_RSDP_128_SMALL_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${CROSS_RSDP_128_SMALL_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'CROSS-rsdp-128-small'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== CROSS_RSDP_128_SMALL_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${CROSS_RSDP_128_SMALL_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'CROSS-rsdp-128-small'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > CROSS_RSDP_128_SMALL_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${CROSS_RSDP_128_SMALL_INFO.keySize.signature} bytes, got ${signature.length}`,
        'CROSS-rsdp-128-small'
      );
    }
  }
