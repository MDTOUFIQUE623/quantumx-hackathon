/**
 * @fileoverview CROSS-rsdp-192-small signature algorithm implementation
 * @module algorithms/sig/cross/cross-rsdp-192-small
 * @description
 * CROSS-rsdp-192-small is a code-based signature scheme providing NIST security level 3.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/cross-rsdp-192-small.deno.js`
    : `../../../../dist/cross-rsdp-192-small.min.js`;
...
/**
 * CROSS-RSDP-192-SMALL-INFO algorithm constants and metadata
 * @type {{readonly name: 'CROSS-rsdp-192-small', readonly identifier: 'CROSS-rsdp-192-small', readonly type: 'sig', readonly securityLevel: 3, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 115, readonly secretKey: 48, readonly signature: 28391}}}
 */
export const CROSS_RSDP_192_SMALL_INFO = {
  name: 'CROSS-rsdp-192-small',
  identifier: 'CROSS-rsdp-192-small',
  type: 'sig',
  securityLevel: 3,
  standardized: false,
  description: 'CROSS-rsdp-192-small code-based signature (NIST Level 3, 192-bit quantum security, small)',
  keySize: {
    publicKey: 115,
    secretKey: 48,
    signature: 28391
...
/**
 * Factory function to create a CROSS-rsdp-192-small signature instance
 *
 * @async
 * @function createCrossRsdp192Small
 * @returns {Promise<CrossRsdp192Small>} Initialized CROSS-rsdp-192-small instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createCrossRsdp192Small } from '@oqs/liboqs-js';
 *
 * const sig = await createCrossRsdp192Small();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createCrossRsdp192Small() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = CROSS_RSDP_192_SMALL_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('CROSS-rsdp-192-small', 'Failed to create SIG instance');
  }

  return new CrossRsdp192Small(wasmModule, sigPtr);
}

/**
 * CROSS-rsdp-192-small signature algorithm wrapper class
 *
 * @class CrossRsdp192Small
 * @description
 * High-level wrapper for CROSS-rsdp-192-small signature operations. Provides secure key generation,
 ...
 *
 * @example
 * const sig = await createCrossRsdp192Small(moduleFactory);
 *
 * // Generate keypair
 * const { publicKey, secretKey } = sig.generateKeyPair();
 *
 ...
 * sig.destroy();
 */
export class CrossRsdp192Small {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(CROSS_RSDP_192_SMALL_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(CROSS_RSDP_192_SMALL_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'CROSS-rsdp-192-small', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Signature (variable length, max 28391 bytes)
   * @throws {LibOQSError} If instance is destroyed
  ...
  sign(message, secretKey) {
    ...
    const secretKeyPtr = this.#wasmModule._malloc(secretKey.length);
    const signaturePtr = this.#wasmModule._malloc(CROSS_RSDP_192_SMALL_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'CROSS-rsdp-192-small', 'Signature generation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof CROSS_RSDP_192_SMALL_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'CROSS-rsdp-192-small'
   * console.log(sig.info.securityLevel);  // 3
   * console.log(sig.info.keySize);        // { publicKey: 115, secretKey: 48, signature: 28391 }
   */
  get info() {
    return CROSS_RSDP_192_SMALL_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'CROSS-rsdp-192-small');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== CROSS_RSDP_192_SMALL_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${CROSS_RSDP_192_SMALL_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'CROSS-rsdp-192-small'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== CROSS_RSDP_192_SMALL_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${CROSS_RSDP_192_SMALL_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'CROSS-rsdp-192-small'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > CROSS_RSDP_192_SMALL_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${CROSS_RSDP_192_SMALL_INFO.keySize.signature} bytes, got ${signature.length}`,
        'CROSS-rsdp-192-small'
      );
    }
  }
