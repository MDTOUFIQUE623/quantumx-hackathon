/**
 * @fileoverview CROSS-rsdp-256-fast signature algorithm implementation
 * @module algorithms/sig/cross/cross-rsdp-256-fast
 * @description
 * CROSS-rsdp-256-fast is a code-based signature scheme providing NIST security level 5.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/cross-rsdp-256-fast.deno.js`
    : `../../../../dist/cross-rsdp-256-fast.min.js`;
...
/**
 * CROSS-RSDP-256-FAST-INFO algorithm constants and metadata
 * @type {{readonly name: 'CROSS-rsdp-256-fast', readonly identifier: 'CROSS-rsdp-256-fast', readonly type: 'sig', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 153, readonly secretKey: 64, readonly signature: 74590}}}
 */
export const CROSS_RSDP_256_FAST_INFO = {
  name: 'CROSS-rsdp-256-fast',
  identifier: 'CROSS-rsdp-256-fast',
  type: 'sig',
  securityLevel: 5,
  standardized: false,
  description: 'CROSS-rsdp-256-fast code-based signature (NIST Level 5, 256-bit quantum security, fast)',
  keySize: {
    publicKey: 153,
    secretKey: 64,
    signature: 74590
...
/**
 * Factory function to create a CROSS-rsdp-256-fast signature instance
 *
 * @async
 * @function createCrossRsdp256Fast
 * @returns {Promise<CrossRsdp256Fast>} Initialized CROSS-rsdp-256-fast instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createCrossRsdp256Fast } from '@oqs/liboqs-js';
 *
 * const sig = await createCrossRsdp256Fast();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createCrossRsdp256Fast() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = CROSS_RSDP_256_FAST_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('CROSS-rsdp-256-fast', 'Failed to create SIG instance');
  }

  return new CrossRsdp256Fast(wasmModule, sigPtr);
}

/**
 * CROSS-rsdp-256-fast signature algorithm wrapper class
 *
 * @class CrossRsdp256Fast
 * @description
 * High-level wrapper for CROSS-rsdp-256-fast signature operations. Provides secure key generation,
 ...
 *
 * @example
 * const sig = await createCrossRsdp256Fast(moduleFactory);
 *
 * // Generate keypair
 * const { publicKey, secretKey } = sig.generateKeyPair();
 *
...
export class CrossRsdp256Fast {
  ...
  /**
   * Generate a new CROSS-rsdp-256-fast keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(CROSS_RSDP_256_FAST_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(CROSS_RSDP_256_FAST_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'CROSS-rsdp-256-fast', 'Key generation failed');
      }
  ...
  /**
   * Sign a message with CROSS-rsdp-256-fast
   *
   * @async
   * @param {Uint8Array} message - Message to sign (any length)
   * @param {Uint8Array} secretKey - Secret key (64 bytes)
   * @returns {Uint8Array} Signature (variable length, max 74590 bytes)
   * @throws {LibOQSError} If instance is destroyed
  ...
  sign(message, secretKey) {
    ...
    const secretKeyPtr = this.#wasmModule._malloc(secretKey.length);
    const signaturePtr = this.#wasmModule._malloc(CROSS_RSDP_256_FAST_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'CROSS-rsdp-256-fast', 'Signature generation failed');
      }
  ...
  /**
   * Verify a CROSS-rsdp-256-fast signature
   *
   * @async
   ...
   *
   * @readonly
   * @returns {typeof CROSS_RSDP_256_FAST_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'CROSS-rsdp-256-fast'
   * console.log(sig.info.securityLevel);  // 5
   * console.log(sig.info.keySize);        // { publicKey: 153, secretKey: 64, signature: 74590 }
   */
  get info() {
    return CROSS_RSDP_256_FAST_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'CROSS-rsdp-256-fast');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== CROSS_RSDP_256_FAST_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${CROSS_RSDP_256_FAST_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'CROSS-rsdp-256-fast'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== CROSS_RSDP_256_FAST_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${CROSS_RSDP_256_FAST_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'CROSS-rsdp-256-fast'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > CROSS_RSDP_256_FAST_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${CROSS_RSDP_256_FAST_INFO.keySize.signature} bytes, got ${signature.length}`,
        'CROSS-rsdp-256-fast'
      );
    }
  }
