/**
 * @fileoverview CROSS-rsdpg-128-small signature algorithm implementation
 * @module algorithms/sig/cross/cross-rsdpg-128-small
 * @description
 * CROSS-rsdpg-128-small is a code-based signature scheme providing NIST security level 1.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/cross-rsdpg-128-small.deno.js`
    : `../../../../dist/cross-rsdpg-128-small.min.js`;
...
/**
 * CROSS-RSDPG-128-SMALL-INFO algorithm constants and metadata
 * @type {{readonly name: 'CROSS-rsdpg-128-small', readonly identifier: 'CROSS-rsdpg-128-small', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 54, readonly secretKey: 32, readonly signature: 8960}}}
 */
export const CROSS_RSDPG_128_SMALL_INFO = {
  name: 'CROSS-rsdpg-128-small',
  identifier: 'CROSS-rsdpg-128-small',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'CROSS-rsdpg-128-small code-based signature (NIST Level 1, 128-bit quantum security, small)',
  keySize: {
    publicKey: 54,
    secretKey: 32,
    signature: 8960
...
/**
 * Factory function to create a CROSS-rsdpg-128-small signature instance
 *
 * @async
 * @function createCrossRsdpg128Small
 * @returns {Promise<CrossRsdpg128Small>} Initialized CROSS-rsdpg-128-small instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createCrossRsdpg128Small } from '@oqs/liboqs-js';
 *
...
export async function createCrossRsdpg128Small() {
  ...
  const algoName = CROSS_RSDPG_128_SMALL_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('CROSS-rsdpg-128-small', 'Failed to create SIG instance');
  }

  return new CrossRsdpg128Small(wasmModule, sigPtr);
}
 ...
 * @description
 * High-level wrapper for CROSS-rsdpg-128-small signature operations. Provides secure key generation,
 ...
 * sig.destroy();
 */
export class CrossRsdpg128Small {
  ...
  generateKeyPair() {
    ...
    const secretKey = new Uint8Array(CROSS_RSDPG_128_SMALL_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'CROSS-rsdpg-128-small', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const secretKeyPtr = this.#wasmModule._malloc(secretKey.length);
    const signaturePtr = this.#wasmModule._malloc(CROSS_RSDPG_128_SMALL_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'CROSS-rsdpg-128-small', 'Signature generation failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof CROSS_RSDPG_128_SMALL_INFO} Algorithm metadata
   *
   * @example
   ...
   */
  get info() {
    return CROSS_RSDPG_128_SMALL_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'CROSS-rsdpg-128-small');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== CROSS_RSDPG_128_SMALL_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${CROSS_RSDPG_128_SMALL_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'CROSS-rsdpg-128-small'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== CROSS_RSDPG_128_SMALL_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${CROSS_RSDPG_128_SMALL_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'CROSS-rsdpg-128-small'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > CROSS_RSDPG_128_SMALL_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${CROSS_RSDPG_128_SMALL_INFO.keySize.signature} bytes, got ${signature.length}`,
        'CROSS-rsdpg-128-small'
      );
    }
  }
