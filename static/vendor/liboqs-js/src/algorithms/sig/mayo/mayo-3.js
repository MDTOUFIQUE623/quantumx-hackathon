/**
 * @fileoverview MAYO-3 signature algorithm implementation
 * @module algorithms/sig/mayo/mayo-3
 * @description
 * MAYO-3 is a multivariate signature scheme based on the Oil and Vinegar problem.
 * It provides compact signatures with NIST security level 3.
 *
 * Key features:
 * - Multivariate cryptography (Oil and Vinegar)
 * - Security Level 3 (192-bit classical, quantum-resistant)
 * - Compact signatures (681 bytes)
 * - Small secret keys (32 bytes)
 *
...
/**
 * MAYO-3-INFO algorithm constants and metadata
 * @type {{readonly name: 'MAYO-3', readonly identifier: 'MAYO-3', readonly type: 'sig', readonly securityLevel: 3, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 2986, readonly secretKey: 32, readonly signature: 681}}}
 */
export const MAYO_3_INFO = {
  name: 'MAYO-3',
  identifier: 'MAYO-3',
  type: 'sig',
  securityLevel: 3,
  standardized: false,
  description: 'MAYO-3 multivariate signature scheme (NIST Level 3, 192-bit quantum security)',
  keySize: {
    publicKey: 2986,
    secretKey: 32,
    signature: 681
...
/**
 * Factory function to create a MAYO-3 signature instance
 *
 * @async
 * @function createMAYO3
 * @returns {Promise<MAYO3>} Initialized MAYO-3 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createMAYO3 } from '@oqs/liboqs-js';
 *
 * const sig = await createMAYO3();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createMAYO3() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = MAYO_3_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('MAYO-3', 'Failed to create SIG instance');
  }

  return new MAYO3(wasmModule, sigPtr);
}

/**
 * MAYO-3 digital signature wrapper class
 *
 * @class MAYO3
 * @description
 * High-level wrapper for MAYO-3 signature operations. Provides secure key generation,
...
export class MAYO3 {
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 2986 bytes
   * console.log('Secret key:', secretKey.length);  // 32 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(MAYO_3_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(MAYO_3_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'MAYO-3', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 681 bytes)
   * @throws {LibOQSError} If instance is destroyed
   ...
   * const signature = sig.sign(message, secretKey);
   * console.log('Signature:', signature.length); // 681 bytes
   */
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(MAYO_3_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'MAYO-3', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (2986 bytes)
   ...
   *
   * @readonly
   * @returns {typeof MAYO_3_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'MAYO-3'
   * console.log(sig.info.securityLevel);  // 3
   * console.log(sig.info.keySize);        // { publicKey: 2986, secretKey: 32, signature: 681 }
   */
  get info() {
    return MAYO_3_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'MAYO-3');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== MAYO_3_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${MAYO_3_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'MAYO-3'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== MAYO_3_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${MAYO_3_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'MAYO-3'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > MAYO_3_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${MAYO_3_INFO.keySize.signature} bytes, got ${signature.length}`,
        'MAYO-3'
      );
    }
  }
