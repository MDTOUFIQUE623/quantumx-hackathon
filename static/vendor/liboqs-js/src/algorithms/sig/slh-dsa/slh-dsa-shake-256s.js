/**
 * @fileoverview SLH-DSA-SHAKE-256s signature algorithm implementation
 * @module algorithms/sig/sphincs/slh-dsa-shake-256s
 * @description
 * SLH-DSA-SHAKE-256s is a stateless hash-based signature scheme providing NIST security level 5.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/slh-dsa-shake-256s.deno.js`
    : `../../../../dist/slh-dsa-shake-256s.min.js`;
...
/**
 * SLH-DSA-SHAKE-256S-INFO algorithm constants and metadata
 * @type {{readonly name: 'SLH-DSA-SHAKE-256s', readonly identifier: 'SLH_DSA_PURE_SHAKE_256S', readonly type: 'sig', readonly securityLevel: 5, readonly standardized: true, readonly description: string, readonly keySize: {readonly publicKey: 64, readonly secretKey: 128, readonly signature: 29792}}}
 */
export const SLH_DSA_SHAKE_256S_INFO = {
  name: 'SLH-DSA-SHAKE-256s',
  identifier: 'SLH_DSA_PURE_SHAKE_256S',
  type: 'sig',
  securityLevel: 5,
  standardized: true,
  description: 'SLH-DSA-SHAKE-256s hash-based signature (NIST Level 5, 256-bit quantum security, SHAKE, small, simple)',
  keySize: {
    publicKey: 64,
    secretKey: 128,
    signature: 29792
...
/**
 * Factory function to create a SLH-DSA-SHAKE-256s signature instance
 *
 * @async
 * @function createSlhDsaShake256s
 * @returns {Promise<SlhDsaShake256s>} Initialized SLH-DSA-SHAKE-256s instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSlhDsaShake256s } from '@oqs/liboqs-js';
 *
 * const sig = await createSlhDsaShake256s();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSlhDsaShake256s() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SLH_DSA_SHAKE_256S_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SLH-DSA-SHAKE-256s', 'Failed to create SIG instance');
  }

  return new SlhDsaShake256s(wasmModule, sigPtr);
}

/**
 * SLH-DSA-SHAKE-256s signature scheme wrapper class
 *
 * @class SlhDsaShake256s
 * @description
 * High-level wrapper for SLH-DSA-SHAKE-256s signature operations. Provides secure key generation,
...
export class SlhDsaShake256s {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(SLH_DSA_SHAKE_256S_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SLH_DSA_SHAKE_256S_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SLH-DSA-SHAKE-256s', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const signatureMaxLen = SLH_DSA_SHAKE_256S_INFO.keySize.signature;
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SLH-DSA-SHAKE-256s', 'Signing failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof SLH_DSA_SHAKE_256S_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'SLH-DSA-SHAKE-256s'
   * console.log(sig.info.securityLevel);  // 5
   * console.log(sig.info.keySize);        // { publicKey: 64, secretKey: 128, signature: 29792 }
   */
  get info() {
    return SLH_DSA_SHAKE_256S_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SLH-DSA-SHAKE-256s');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SLH_DSA_SHAKE_256S_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SLH_DSA_SHAKE_256S_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SLH-DSA-SHAKE-256s'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SLH_DSA_SHAKE_256S_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SLH_DSA_SHAKE_256S_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SLH-DSA-SHAKE-256s'
      );
    }
  }
  ...
  #validateSignature(signature) {
    if (!isUint8Array(signature) || signature.length === 0 || signature.length > SLH_DSA_SHAKE_256S_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature: expected 0 < length <= ${SLH_DSA_SHAKE_256S_INFO.keySize.signature} bytes, got ${signature?.length ?? 'null'}`,
        'SLH-DSA-SHAKE-256s'
      );
    }
  }
