/**
 * @fileoverview SNOVA-24-5-4-SHAKE signature algorithm implementation
 * @module algorithms/sig/snova/snova-24-5-4-shake
 * @description
 * SNOVA-24-5-4-SHAKE is a multivariate quadratic signature scheme from the SNOVA family.
 * This is the SHAKE variant.
 *
 * Key features:
...
export const SNOVA_24_5_4_SHAKE_INFO = {
  ...
  securityLevel: 1,
  standardized: false,
  description: 'SNOVA-24-5-4-SHAKE multivariate signature scheme (NIST Level 1, 128-bit quantum security, SHAKE variant)',
  keySize: {
    publicKey: 1016,
    secretKey: 48,
    signature: 248
...
/**
 * Factory function to create a SNOVA-24-5-4-SHAKE signature instance
 *
 * @async
 * @function createSnova2454Shake
 * @returns {Promise<Snova2454Shake>} Initialized SNOVA-24-5-4-SHAKE instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSnova2454Shake } from '@oqs/liboqs-js';
 *
 * const sig = await createSnova2454Shake();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSnova2454Shake() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SNOVA_24_5_4_SHAKE_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SNOVA-24-5-4-SHAKE', 'Failed to create SIG instance');
  }

  return new Snova2454Shake(wasmModule, sigPtr);
}
 ...
 * @description
 * High-level wrapper for SNOVA-24-5-4-SHAKE signature operations. Provides secure key generation,
...
export class Snova2454Shake {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(SNOVA_24_5_4_SHAKE_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SNOVA_24_5_4_SHAKE_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SNOVA-24-5-4-SHAKE', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(SNOVA_24_5_4_SHAKE_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SNOVA-24-5-4-SHAKE', 'Signing failed');
      }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SNOVA-24-5-4-SHAKE');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SNOVA_24_5_4_SHAKE_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SNOVA_24_5_4_SHAKE_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SNOVA-24-5-4-SHAKE'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SNOVA_24_5_4_SHAKE_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SNOVA_24_5_4_SHAKE_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SNOVA-24-5-4-SHAKE'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > SNOVA_24_5_4_SHAKE_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${SNOVA_24_5_4_SHAKE_INFO.keySize.signature} bytes, got ${signature.length}`,
        'SNOVA-24-5-4-SHAKE'
      );
    }
  }
