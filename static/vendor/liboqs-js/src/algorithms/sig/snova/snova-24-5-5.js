/**
 * @fileoverview SNOVA-24-5-5 signature algorithm implementation
 * @module algorithms/sig/snova/snova-24-5-5
 * @description
 * SNOVA-24-5-5 is a multivariate quadratic signature scheme from the SNOVA family.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/snova-24-5-5.deno.js`
    : `../../../../dist/snova-24-5-5.min.js`;
...
/**
 * SNOVA-24-5-5-INFO algorithm constants and metadata
 * @type {{readonly name: 'SNOVA-24-5-5', readonly identifier: 'SNOVA_24_5_5', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 1579, readonly secretKey: 48, readonly signature: 379}}}
 */
export const SNOVA_24_5_5_INFO = {
  name: 'SNOVA-24-5-5',
  identifier: 'SNOVA_24_5_5',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'SNOVA-24-5-5 multivariate signature scheme (NIST Level 1, 128-bit quantum security, standard variant)',
  keySize: {
    publicKey: 1579,
    secretKey: 48,
    signature: 379
...
/**
 * Factory function to create a SNOVA-24-5-5 signature instance
 *
 * @async
 * @function createSnova2455
 * @returns {Promise<Snova2455>} Initialized SNOVA-24-5-5 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSnova2455 } from '@oqs/liboqs-js';
 *
 * const sig = await createSnova2455();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSnova2455() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SNOVA_24_5_5_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SNOVA-24-5-5', 'Failed to create SIG instance');
  }

  return new Snova2455(wasmModule, sigPtr);
}

/**
 * SNOVA-24-5-5 digital signature wrapper class
 *
 * @class Snova2455
 * @description
 * High-level wrapper for SNOVA-24-5-5 signature operations. Provides secure key generation,
...
export class Snova2455 {
  ...
  /**
   * Generate a new SNOVA-24-5-5 keypair
   *
   * @async
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 1579 bytes
   * console.log('Secret key:', secretKey.length);  // 48 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(SNOVA_24_5_5_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SNOVA_24_5_5_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SNOVA-24-5-5', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 379 bytes)
   * @throws {LibOQSError} If instance is destroyed
   ...
   * const signature = sig.sign(message, secretKey);
   * console.log('Signature:', signature.length); // 379 bytes
   */
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(SNOVA_24_5_5_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SNOVA-24-5-5', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (1579 bytes)
   ...
   *
   * @readonly
   * @returns {typeof SNOVA_24_5_5_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'SNOVA-24-5-5'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 1579, secretKey: 48, signature: 379 }
   */
  get info() {
    return SNOVA_24_5_5_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SNOVA-24-5-5');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SNOVA_24_5_5_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SNOVA_24_5_5_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SNOVA-24-5-5'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SNOVA_24_5_5_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SNOVA_24_5_5_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SNOVA-24-5-5'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > SNOVA_24_5_5_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${SNOVA_24_5_5_INFO.keySize.signature} bytes, got ${signature.length}`,
        'SNOVA-24-5-5'
      );
    }
  }
