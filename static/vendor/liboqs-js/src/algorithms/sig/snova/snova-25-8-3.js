/**
 * @fileoverview SNOVA-25-8-3 signature algorithm implementation
 * @module algorithms/sig/snova/snova-25-8-3
 * @description
 * SNOVA-25-8-3 is a multivariate quadratic signature scheme from the SNOVA family.
...
export const SNOVA_25_8_3_INFO = {
  name: 'SNOVA-25-8-3',
  identifier: 'SNOVA_25_8_3',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'SNOVA-25-8-3 multivariate signature scheme (NIST Level 1, 128-bit quantum security, standard variant)',
  keySize: {
    publicKey: 2320,
    secretKey: 48,
    signature: 165
...
/**
 * Factory function to create a SNOVA-25-8-3 signature instance
 *
 * @async
 * @function createSnova2583
 * @returns {Promise<Snova2583>} Initialized SNOVA-25-8-3 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSnova2583 } from '@oqs/liboqs-js';
 *
 * const sig = await createSnova2583();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSnova2583() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SNOVA_25_8_3_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SNOVA-25-8-3', 'Failed to create SIG instance');
  }

  return new Snova2583(wasmModule, sigPtr);
}

/**
 * SNOVA-25-8-3 digital signature wrapper class
 *
 * @class Snova2583
 * @description
 * High-level wrapper for SNOVA-25-8-3 signature operations. Provides secure key generation,
...
export class Snova2583 {
  ...
  /**
   * Generate a new SNOVA-25-8-3 keypair
   *
   * @async
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 2320 bytes
   * console.log('Secret key:', secretKey.length);  // 48 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(SNOVA_25_8_3_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SNOVA_25_8_3_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SNOVA-25-8-3', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 165 bytes)
   * @throws {LibOQSError} If instance is destroyed
   ...
   * const signature = sig.sign(message, secretKey);
   * console.log('Signature:', signature.length); // 165 bytes
   */
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(SNOVA_25_8_3_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SNOVA-25-8-3', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (2320 bytes)
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SNOVA-25-8-3');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SNOVA_25_8_3_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SNOVA_25_8_3_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SNOVA-25-8-3'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SNOVA_25_8_3_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SNOVA_25_8_3_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SNOVA-25-8-3'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > SNOVA_25_8_3_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${SNOVA_25_8_3_INFO.keySize.signature} bytes, got ${signature.length}`,
        'SNOVA-25-8-3'
      );
    }
  }
