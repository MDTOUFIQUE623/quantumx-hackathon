/**
 * @fileoverview SNOVA-37-8-4 signature algorithm implementation
 * @module algorithms/sig/snova/snova-37-8-4
 * @description
 * SNOVA-37-8-4 is a multivariate quadratic signature scheme from the SNOVA family.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/snova-37-8-4.deno.js`
    : `../../../../dist/snova-37-8-4.min.js`;
...
/**
 * SNOVA-37-8-4-INFO algorithm constants and metadata
 * @type {{readonly name: 'SNOVA-37-8-4', readonly identifier: 'SNOVA_37_8_4', readonly type: 'sig', readonly securityLevel: 3, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 4112, readonly secretKey: 48, readonly signature: 376}}}
 */
export const SNOVA_37_8_4_INFO = {
  name: 'SNOVA-37-8-4',
  identifier: 'SNOVA_37_8_4',
  type: 'sig',
  securityLevel: 3,
  standardized: false,
  description: 'SNOVA-37-8-4 multivariate signature scheme (NIST Level 3, 192-bit quantum security, standard variant)',
  keySize: {
    publicKey: 4112,
    secretKey: 48,
    signature: 376
...
/**
 * Factory function to create a SNOVA-37-8-4 signature instance
 *
 * @async
 * @function createSnova3784
 * @returns {Promise<Snova3784>} Initialized SNOVA-37-8-4 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSnova3784 } from '@oqs/liboqs-js';
 *
 * const sig = await createSnova3784();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSnova3784() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SNOVA_37_8_4_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SNOVA-37-8-4', 'Failed to create SIG instance');
  }

  return new Snova3784(wasmModule, sigPtr);
}
 ...
 * @class Snova3784
 * @description
 * High-level wrapper for SNOVA-37-8-4 signature operations. Provides secure key generation,
...
export class Snova3784 {
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 4112 bytes
   * console.log('Secret key:', secretKey.length);  // 48 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(SNOVA_37_8_4_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SNOVA_37_8_4_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SNOVA-37-8-4', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 376 bytes)
   * @throws {LibOQSError} If instance is destroyed
   ...
   * const signature = sig.sign(message, secretKey);
   * console.log('Signature:', signature.length); // 376 bytes
   */
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(SNOVA_37_8_4_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SNOVA-37-8-4', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (4112 bytes)
   ...
   *
   * @readonly
   * @returns {typeof SNOVA_37_8_4_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'SNOVA-37-8-4'
   * console.log(sig.info.securityLevel);  // 3
   * console.log(sig.info.keySize);        // { publicKey: 4112, secretKey: 48, signature: 376 }
   */
  get info() {
    return SNOVA_37_8_4_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SNOVA-37-8-4');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SNOVA_37_8_4_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SNOVA_37_8_4_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SNOVA-37-8-4'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SNOVA_37_8_4_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SNOVA_37_8_4_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SNOVA-37-8-4'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > SNOVA_37_8_4_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${SNOVA_37_8_4_INFO.keySize.signature} bytes, got ${signature.length}`,
        'SNOVA-37-8-4'
      );
    }
  }
