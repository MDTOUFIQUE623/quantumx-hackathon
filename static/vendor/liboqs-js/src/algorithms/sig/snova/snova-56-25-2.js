/**
 * @fileoverview SNOVA-56-25-2 signature algorithm implementation
 * @module algorithms/sig/snova/snova-56-25-2
 * @description
 * SNOVA-56-25-2 is a multivariate quadratic signature scheme from the SNOVA family.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/snova-56-25-2.deno.js`
    : `../../../../dist/snova-56-25-2.min.js`;
...
/**
 * SNOVA-56-25-2-INFO algorithm constants and metadata
 * @type {{readonly name: 'SNOVA-56-25-2', readonly identifier: 'SNOVA_56_25_2', readonly type: 'sig', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 31266, readonly secretKey: 48, readonly signature: 178}}}
 */
export const SNOVA_56_25_2_INFO = {
  name: 'SNOVA-56-25-2',
  identifier: 'SNOVA_56_25_2',
  type: 'sig',
  securityLevel: 5,
  standardized: false,
  description: 'SNOVA-56-25-2 multivariate signature scheme (NIST Level 5, 256-bit quantum security, standard variant)',
  keySize: {
    publicKey: 31266,
    secretKey: 48,
    signature: 178
...
/**
 * Factory function to create a SNOVA-56-25-2 signature instance
 *
 * @async
 * @function createSnova56252
 * @returns {Promise<Snova56252>} Initialized SNOVA-56-25-2 instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createSnova56252 } from '@oqs/liboqs-js';
 *
 * const sig = await createSnova56252();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createSnova56252() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = SNOVA_56_25_2_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('SNOVA-56-25-2', 'Failed to create SIG instance');
  }

  return new Snova56252(wasmModule, sigPtr);
}

/**
 * SNOVA-56-25-2 digital signature wrapper class
 *
 * @class Snova56252
 * @description
 * High-level wrapper for SNOVA-56-25-2 signature operations. Provides secure key generation,
...
export class Snova56252 {
  ...
  /**
   * Generate a new SNOVA-56-25-2 keypair
   *
   * @async
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 31266 bytes
   * console.log('Secret key:', secretKey.length);  // 48 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(SNOVA_56_25_2_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(SNOVA_56_25_2_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'SNOVA-56-25-2', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 178 bytes)
   * @throws {LibOQSError} If instance is destroyed
   ...
   * const signature = sig.sign(message, secretKey);
   * console.log('Signature:', signature.length); // 178 bytes
   */
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(SNOVA_56_25_2_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'SNOVA-56-25-2', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (31266 bytes)
   ...
   *
   * @readonly
   * @returns {typeof SNOVA_56_25_2_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'SNOVA-56-25-2'
   * console.log(sig.info.securityLevel);  // 5
   * console.log(sig.info.keySize);        // { publicKey: 31266, secretKey: 48, signature: 178 }
   */
  get info() {
    return SNOVA_56_25_2_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'SNOVA-56-25-2');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== SNOVA_56_25_2_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${SNOVA_56_25_2_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'SNOVA-56-25-2'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== SNOVA_56_25_2_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${SNOVA_56_25_2_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'SNOVA-56-25-2'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > SNOVA_56_25_2_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${SNOVA_56_25_2_INFO.keySize.signature} bytes, got ${signature.length}`,
        'SNOVA-56-25-2'
      );
    }
  }
