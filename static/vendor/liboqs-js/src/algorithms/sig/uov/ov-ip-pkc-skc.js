/**
 * @fileoverview OV-Ip-pkc-skc signature algorithm implementation
 * @module algorithms/sig/uov/ov-ip-pkc-skc
 * @description
 * OV-Ip-pkc-skc is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
export const OV_IP_PKC_SKC_INFO = {
  name: 'OV-Ip-pkc-skc',
  identifier: 'OV-Ip-pkc-skc',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'OV-Ip-pkc-skc UOV signature scheme (NIST Level 1, 128-bit quantum security, compressed public and secret keys)',
  keySize: {
    publicKey: 43576,
    secretKey: 32,
    signature: 128
 ...
 *
 * @async
 * @function createOVIpPkcSkc
 * @returns {Promise<OVIpPkcSkc>} Initialized OV-Ip-pkc-skc instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVIpPkcSkc } from '@oqs/liboqs-js';
 *
 * const sig = await createOVIpPkcSkc();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createOVIpPkcSkc() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = OV_IP_PKC_SKC_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-Ip-pkc-skc', 'Failed to create SIG instance');
  }

  return new OVIpPkcSkc(wasmModule, sigPtr);
}

/**
 * OV-Ip-pkc-skc digital signature wrapper class
 *
 * @class OVIpPkcSkc
 * @description
 * High-level wrapper for OV-Ip-pkc-skc signature operations. Provides secure key generation,
...
export class OVIpPkcSkc {
  ...
  /**
   * Generate a new OV-Ip-pkc-skc keypair
   *
   * @async
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(OV_IP_PKC_SKC_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_IP_PKC_SKC_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-Ip-pkc-skc', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_IP_PKC_SKC_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-Ip-pkc-skc', 'Signing failed');
      }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-Ip-pkc-skc');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_IP_PKC_SKC_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_IP_PKC_SKC_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-Ip-pkc-skc'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_IP_PKC_SKC_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_IP_PKC_SKC_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-Ip-pkc-skc'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_IP_PKC_SKC_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_IP_PKC_SKC_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-Ip-pkc-skc'
      );
    }
  }
