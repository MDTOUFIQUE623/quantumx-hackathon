/**
 * @fileoverview OV-Ip-pkc signature algorithm implementation
 * @module algorithms/sig/uov/ov-ip-pkc
 * @description
 * OV-Ip-pkc is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/ov-ip-pkc.deno.js`
    : `../../../../dist/ov-ip-pkc.min.js`;
...
/**
 * OV-IP-PKC-INFO algorithm constants and metadata
 * @type {{readonly name: 'OV-Ip-pkc', readonly identifier: 'OV-Ip-pkc', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 43576, readonly secretKey: 237896, readonly signature: 128}}}
 */
export const OV_IP_PKC_INFO = {
  name: 'OV-Ip-pkc',
  identifier: 'OV-Ip-pkc',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'OV-Ip-pkc UOV signature scheme (NIST Level 1, 128-bit quantum security, compressed public key)',
  keySize: {
    publicKey: 43576,
    secretKey: 237896,
    signature: 128
 ...
 *
 * @async
 * @function createOVIpPkc
 * @returns {Promise<OVIpPkc>} Initialized OV-Ip-pkc instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVIpPkc } from '@oqs/liboqs-js';
 *
 * const sig = await createOVIpPkc();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createOVIpPkc() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = OV_IP_PKC_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-Ip-pkc', 'Failed to create SIG instance');
  }

  return new OVIpPkc(wasmModule, sigPtr);
}

/**
 * OV-Ip-pkc digital signature wrapper class
 *
 * @class OVIpPkc
 * @description
 * High-level wrapper for OV-Ip-pkc signature operations. Provides secure key generation,
...
export class OVIpPkc {
  ...
  /**
   * Generate a new OV-Ip-pkc keypair
   *
   * @async
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 43576 bytes
   * console.log('Secret key:', secretKey.length);  // 237896 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(OV_IP_PKC_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_IP_PKC_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-Ip-pkc', 'Key generation failed');
      }
   ...
   * @returns {Uint8Array} Digital signature (up to 128 bytes)
   * @throws {LibOQSError} If instance is destroyed
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_IP_PKC_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-Ip-pkc', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (43576 bytes)
   ...
   *
   * @readonly
   * @returns {typeof OV_IP_PKC_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'OV-Ip-pkc'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 43576, secretKey: 237896, signature: 128 }
   */
  get info() {
    return OV_IP_PKC_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-Ip-pkc');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_IP_PKC_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_IP_PKC_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-Ip-pkc'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_IP_PKC_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_IP_PKC_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-Ip-pkc'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_IP_PKC_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_IP_PKC_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-Ip-pkc'
      );
    }
  }
