/**
 * @fileoverview OV-Is-pkc signature algorithm implementation
 * @module algorithms/sig/uov/ov-is-pkc
 * @description
 * OV-Is-pkc is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/ov-is-pkc.deno.js`
    : `../../../../dist/ov-is-pkc.min.js`;
...
/**
 * OV-IS-PKC-INFO algorithm constants and metadata
 * @type {{readonly name: 'OV-Is-pkc', readonly identifier: 'OV-Is-pkc', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 66576, readonly secretKey: 348704, readonly signature: 96}}}
 */
export const OV_IS_PKC_INFO = {
  name: 'OV-Is-pkc',
  identifier: 'OV-Is-pkc',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'OV-Is-pkc UOV signature scheme (NIST Level 1, 128-bit quantum security, compressed public key)',
  keySize: {
    publicKey: 66576,
    secretKey: 348704,
    signature: 96
 ...
 *
 * @async
 * @function createOVIsPkc
 * @returns {Promise<OVIsPkc>} Initialized OV-Is-pkc instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVIsPkc } from '@oqs/liboqs-js';
 *
 * const sig = await createOVIsPkc();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createOVIsPkc() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = OV_IS_PKC_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-Is-pkc', 'Failed to create SIG instance');
  }

  return new OVIsPkc(wasmModule, sigPtr);
}

/**
 * OV-Is-pkc digital signature wrapper class
 *
 * @class OVIsPkc
 * @description
 * High-level wrapper for OV-Is-pkc signature operations. Provides secure key generation,
...
export class OVIsPkc {
  ...
  /**
   * Generate a new OV-Is-pkc keypair
   *
   * @async
   ...
   * console.log('Secret key:', secretKey.length);  // 348704 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(OV_IS_PKC_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_IS_PKC_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-Is-pkc', 'Key generation failed');
      }
   ...
   * @param {Uint8Array} secretKey - Secret key for signing (348704 bytes)
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_IS_PKC_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-Is-pkc', 'Signing failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof OV_IS_PKC_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'OV-Is-pkc'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 66576, secretKey: 348704, signature: 96 }
   */
  get info() {
    return OV_IS_PKC_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-Is-pkc');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_IS_PKC_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_IS_PKC_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-Is-pkc'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_IS_PKC_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_IS_PKC_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-Is-pkc'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_IS_PKC_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_IS_PKC_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-Is-pkc'
      );
    }
  }
