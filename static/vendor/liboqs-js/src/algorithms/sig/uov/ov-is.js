/**
 * @fileoverview OV-Is signature algorithm implementation
 * @module algorithms/sig/uov/ov-is
 * @description
 * OV-Is is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
/**
 * OV-IS-INFO algorithm constants and metadata
 * @type {{readonly name: 'OV-Is', readonly identifier: 'OV-Is', readonly type: 'sig', readonly securityLevel: 1, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 412160, readonly secretKey: 348704, readonly signature: 96}}}
 */
export const OV_IS_INFO = {
  name: 'OV-Is',
  identifier: 'OV-Is',
  type: 'sig',
  securityLevel: 1,
  standardized: false,
  description: 'OV-Is UOV signature scheme (NIST Level 1, 128-bit quantum security, uncompressed keys)',
  keySize: {
    publicKey: 412160,
    secretKey: 348704,
    signature: 96
...
/**
 * Factory function to create a OV-Is signature instance
 *
 * @async
 * @function createOVIs
 * @returns {Promise<OVIs>} Initialized OV-Is instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVIs } from '@oqs/liboqs-js';
 *
 * const sig = await createOVIs();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createOVIs() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = OV_IS_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-Is', 'Failed to create SIG instance');
  }

  return new OVIs(wasmModule, sigPtr);
}

/**
 * OV-Is digital signature wrapper class
 *
 * @class OVIs
 * @description
 * High-level wrapper for OV-Is signature operations. Provides secure key generation,
...
export class OVIs {
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 412160 bytes
   * console.log('Secret key:', secretKey.length);  // 348704 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(OV_IS_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_IS_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-Is', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_IS_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-Is', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (412160 bytes)
   ...
   *
   * @readonly
   * @returns {typeof OV_IS_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'OV-Is'
   * console.log(sig.info.securityLevel);  // 1
   * console.log(sig.info.keySize);        // { publicKey: 412160, secretKey: 348704, signature: 96 }
   */
  get info() {
    return OV_IS_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-Is');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_IS_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_IS_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-Is'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_IS_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_IS_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-Is'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_IS_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_IS_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-Is'
      );
    }
  }
