/**
 * @fileoverview OV-V-pkc signature algorithm implementation
 * @module algorithms/sig/uov/ov-v-pkc
 * @description
 * OV-V-pkc is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
async function loadModule() {
  ...
  const modulePath = isDeno
    ? `../../../../dist/ov-v-pkc.deno.js`
    : `../../../../dist/ov-v-pkc.min.js`;
...
/**
 * OV-V-PKC-INFO algorithm constants and metadata
 * @type {{readonly name: 'OV-V-pkc', readonly identifier: 'OV-V-pkc', readonly type: 'sig', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 446992, readonly secretKey: 2436704, readonly signature: 260}}}
 */
export const OV_V_PKC_INFO = {
  name: 'OV-V-pkc',
  identifier: 'OV-V-pkc',
  type: 'sig',
  securityLevel: 5,
  standardized: false,
  description: 'OV-V-pkc UOV signature scheme (NIST Level 5, 256-bit quantum security, compressed public key)',
  keySize: {
    publicKey: 446992,
    secretKey: 2436704,
    signature: 260
...
/**
 * Factory function to create a OV-V-pkc signature instance
 *
 * @async
 * @function createOVVPkc
 * @returns {Promise<OVVPkc>} Initialized OV-V-pkc instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVVPkc } from '@oqs/liboqs-js';
 *
 * const sig = await createOVVPkc();
 * const { publicKey, secretKey } = sig.generateKeyPair();
 * sig.destroy();
 */
export async function createOVVPkc() {
  const moduleFactory = await loadModule();
  const wasmModule = await moduleFactory();
  wasmModule._OQS_init();

  const algoName = OV_V_PKC_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-V-pkc', 'Failed to create SIG instance');
  }

  return new OVVPkc(wasmModule, sigPtr);
}
 ...
 * @class OVVPkc
 * @description
 * High-level wrapper for OV-V-pkc signature operations. Provides secure key generation,
...
export class OVVPkc {
  ...
  generateKeyPair() {
    ...
    const publicKey = new Uint8Array(OV_V_PKC_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_V_PKC_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-V-pkc', 'Key generation failed');
      }
   ...
   * @param {Uint8Array} secretKey - Secret key for signing (2436704 bytes)
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_V_PKC_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-V-pkc', 'Signing failed');
      }
   ...
   *
   * @readonly
   * @returns {typeof OV_V_PKC_INFO} Algorithm metadata
   *
   * @example
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-V-pkc');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_V_PKC_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_V_PKC_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-V-pkc'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_V_PKC_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_V_PKC_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-V-pkc'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_V_PKC_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_V_PKC_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-V-pkc'
      );
    }
  }
