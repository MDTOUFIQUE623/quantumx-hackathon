/**
 * @fileoverview OV-V signature algorithm implementation
 * @module algorithms/sig/uov/ov-v
 * @description
 * OV-V is an Unbalanced Oil and Vinegar (UOV) signature scheme.
...
/**
 * OV-V-INFO algorithm constants and metadata
 * @type {{readonly name: 'OV-V', readonly identifier: 'OV-V', readonly type: 'sig', readonly securityLevel: 5, readonly standardized: false, readonly description: string, readonly keySize: {readonly publicKey: 2869440, readonly secretKey: 2436704, readonly signature: 260}}}
 */
export const OV_V_INFO = {
  name: 'OV-V',
  identifier: 'OV-V',
  type: 'sig',
  securityLevel: 5,
  standardized: false,
  description: 'OV-V UOV signature scheme (NIST Level 5, 256-bit quantum security, uncompressed keys)',
  keySize: {
    publicKey: 2869440,
    secretKey: 2436704,
    signature: 260
...
/**
 * Factory function to create a OV-V signature instance
 *
 * @async
 * @function createOVV
 * @returns {Promise<OVV>} Initialized OV-V instance
 * @throws {LibOQSInitError} If module initialization fails
 *
 * @example
 * import { createOVV } from '@oqs/liboqs-js';
 *
...
export async function createOVV() {
  ...
  const algoName = OV_V_INFO.identifier;
  ...
  if (!sigPtr) {
    throw new LibOQSInitError('OV-V', 'Failed to create SIG instance');
  }

  return new OVV(wasmModule, sigPtr);
}
 ...
 * @class OVV
 * @description
 * High-level wrapper for OV-V signature operations. Provides secure key generation,
...
export class OVV {
   ...
   * @example
   * const { publicKey, secretKey } = sig.generateKeyPair();
   * console.log('Public key:', publicKey.length);  // 2869440 bytes
   * console.log('Secret key:', secretKey.length);  // 2436704 bytes
   */
  generateKeyPair() {
    this.#checkDestroyed();

    const publicKey = new Uint8Array(OV_V_INFO.keySize.publicKey);
    const secretKey = new Uint8Array(OV_V_INFO.keySize.secretKey);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('generateKeyPair', 'OV-V', 'Key generation failed');
      }
  ...
  sign(message, secretKey) {
    ...
    const signature = new Uint8Array(OV_V_INFO.keySize.signature);
    ...
    try {
      ...
      if (result !== 0) {
        throw new LibOQSOperationError('sign', 'OV-V', 'Signing failed');
      }
   ...
   * @param {Uint8Array} publicKey - Public key for verification (2869440 bytes)
   ...
   *
   * @readonly
   * @returns {typeof OV_V_INFO} Algorithm metadata
   *
   * @example
   * console.log(sig.info.name);           // 'OV-V'
   * console.log(sig.info.securityLevel);  // 5
   * console.log(sig.info.keySize);        // { publicKey: 2869440, secretKey: 2436704, signature: 260 }
   */
  get info() {
    return OV_V_INFO;
  }
  ...
  #checkDestroyed() {
    if (this.#destroyed) {
      throw new LibOQSError('Instance has been destroyed', 'OV-V');
    }
  }
  ...
  #validatePublicKey(publicKey) {
    if (!isUint8Array(publicKey) || publicKey.length !== OV_V_INFO.keySize.publicKey) {
      throw new LibOQSValidationError(
        `Invalid public key: expected ${OV_V_INFO.keySize.publicKey} bytes, got ${publicKey?.length ?? 'null'}`,
        'OV-V'
      );
    }
  }
  ...
  #validateSecretKey(secretKey) {
    if (!isUint8Array(secretKey) || secretKey.length !== OV_V_INFO.keySize.secretKey) {
      throw new LibOQSValidationError(
        `Invalid secret key: expected ${OV_V_INFO.keySize.secretKey} bytes, got ${secretKey?.length ?? 'null'}`,
        'OV-V'
      );
    }
  }
  ...
  #validateSignature(signature) {
    ...
    if (signature.length === 0 || signature.length > OV_V_INFO.keySize.signature) {
      throw new LibOQSValidationError(
        `Invalid signature length: expected up to ${OV_V_INFO.keySize.signature} bytes, got ${signature.length}`,
        'OV-V'
      );
    }
  }
